...
Runs locally with zero cloud setup, or connect to the hosted version at `https://portfolio-analyzer-mcp.onrender.com/mcp`.
...
Point Claude Desktop at the live server:
...
```json
{
  "mcpServers": {
    "portfolio-analyzer": {
      ...
      "url": "https://portfolio-analyzer-mcp.onrender.com/mcp"
    }
  }
}
```

Since there's no local file, pass your holdings as a JSON string when calling `get_portfolio` or `get_portfolio_metrics`:

```json
{"holdings": [{"ticker": "AAPL", "shares": 10, "purchase_price": 150.0, "purchase_date": "2024-01-15"}]}
```

All other tools — technicals, fundamentals, news, SEC filings, compare — only need a ticker and work without any portfolio data.

---

### Option B — Run Locally (stdio)

Best if you want your portfolio data to stay on your machine.
